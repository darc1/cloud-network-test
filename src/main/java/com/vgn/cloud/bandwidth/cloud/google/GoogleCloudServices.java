package com.vgn.cloud.bandwidth.cloud.google;
...
public class GoogleCloudServices implements ICloudServices{
    ...
    public List<VMInstance> createVMs(List<VMTestInfo> vms) throws IOException, InterruptedException {
        ...
        Compute.Snapshots.Get snapshots = compute.snapshots().get(projectId, snapshotName);
    ...
    private String createVM(Snapshot snapshot, VMTestInfo vmInfo) throws IOException, InterruptedException {
        ...
        String machineTypeLink = getMachineType(vmInfo.getRegion());
        ...
        Compute.Instances.Insert insert = compute.instances().insert(projectId, vmInfo.getRegion(), newInstance);
    ...
    private String getMachineType(String region) throws IOException {
        String machineType = "n1-standard-1";
        Compute.MachineTypes.Get machine = compute.machineTypes().get(projectId, region, machineType);
    ...
    private List<NetworkInterface> createNetworkInterface(String region) throws IOException {
        ...
        Compute.Subnetworks.List subnets = compute.subnetworks().list(projectId,
    ...
    private List<AttachedDisk> createDiskFromSnapshot(Snapshot snapshot, String region, String deviceName) throws IOException {
        ...
        Compute.Disks.Insert disks = compute.disks().insert(projectId, region, disk1);
        Operation disk = disks.execute();

        Compute.Disks.Get getDisk = compute.disks().get(projectId, region, deviceName);
